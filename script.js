...
function jaExiste(cod, nome, email) {
  const alunoExistente = alunos.find(
    (aluno) => aluno.cod === cod || aluno.nome === nome || aluno.email === email
  )
  if (alunoExistente) {
...
function salvarDados() {
  ...
  const opcaoSelecionadaValue = document.querySelector(
    'input[name="grupo_opcoes"]:checked'
  ).value
  ...
  try {
    ...
    const aluno = {
      ...
      numerosArray: [
        Number(n1.value.trim()),
        Number(n2.value.trim()),
        Number(n3.value.trim()),
      ],
...
function pesquisarAluno() {
  ...
  try {
    ...
    const tabelaAlunoEncontrado = document
      .getElementById("tabelaValores")
      .querySelector("tbody")
